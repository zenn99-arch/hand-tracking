...
def concat_tile(im_list_2d, size):
    ...
    flattened_images = [img for sublist in im_list_2d for img in sublist]
    
    # Calculate number of rows and columns
    num_rows = min(size[1], len(flattened_images) // size[0] + (1 if len(flattened_images) % size[0] != 0 else 0))
    num_cols = min(size[0], len(flattened_images))

    # Create list of rows for concatenation
    all_imgs = []
    for row in range(num_rows):
        start_idx = row * size[0]
        end_idx = start_idx + size[0]
        imgs = flattened_images[start_idx:end_idx]
        # Pad the row with empty images if necessary
        if len(imgs) < size[0]:
            imgs.extend([np.zeros_like(im_list_2d[0][0])] * (size[0] - len(imgs)))
